...
import {
  buildNotebookSource, buildBundleSource, buildSubjectSource, buildNotesSetSource,
  getNotebookTransport,
} from "@/lib/notebooklm";
import { NotebookSourceInput } from "@/lib/notebooklm/schema";
...
export async function sendToNotebookLM(
  raw: unknown,
  transportId: NotebookTransportId
): Promise<{ ok: boolean; message: string; record: NotebookExportRec }> {
  const input = NotebookSourceInput.parse(raw);
  let built: { title: string; body: string };
  switch (input.kind) {
    case "bundle":    built = await buildBundleSource(input.sourceId); break;
    case "subject":   built = await buildSubjectSource(input.sourceId); break;
    case "notes-set": built = await buildNotesSetSource(input.sourceId.split(",")); break;
    case "full-export": {
      const full = await exportAllData();
      built = { title: input.title || "Full StudyMax export", body: "```json\n" + full + "\n```" };
      break;
    }
  }
  const source = buildNotebookSource(input, built.body);
  const transport = getNotebookTransport(transportId);

  if (!(await transport.isAvailable())) {
    return {
      ok: false,
      message: transportId === "share-link"
        ? "Share link is disabled in Settings (or you're offline). Use Download .md instead."
        : "This transport is not available right now.",
      record: { id: uid(), kind: input.kind, sourceId: input.sourceId, title: built.title, byteSize: 0, transport: transportId, createdAt: new Date() },
    };
  }

  const result = await transport.send(input, source);
  const record: NotebookExportRec = {
    id: uid(),
    kind: input.kind,
  ...
  await db.notebookExports.add(record);
  return { ok: result.ok, message: result.message, record };
}

export async function getRecentNotebookExports(limit = 5): Promise<NotebookExportRec[]> {
  return db.notebookExports.orderBy("createdAt").reverse().limit(limit).toArray();
}

export async function redownloadNotebookExport(exportId: string): Promise<boolean> {
  const rec = await db.notebookExports.get(exportId);
  if (!rec) return false;
  return (await sendToNotebookLM({
    kind: rec.kind, sourceId: rec.sourceId, title: rec.title,
    includeCards: true, includeNotes: true, includeSessions: false,
  }, "file-download")).ok;
}

export async function getNotebookSettings(): Promise<NotebookSettings> {
  const row = await db.notebookSettings.get("singleton");
  return row ?? { id: "singleton", shareLinkEnabled: false };
}
